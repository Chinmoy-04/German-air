...
function useNumberFlowElementReady(): boolean {
  // Always start `false` so the very first client render matches the
  // server-rendered (static text) markup exactly. Flipping this during the
  // lazy `useState` initializer reads live browser globals that can already
  // differ by the time hydration runs, which caused a hydration mismatch.
